...
AppRuntimeWebViewHostImpl::AppRuntimeWebViewHostImpl(
    ...
    : webview_host_bindings_(web_contents, this),
      blink_delegate_bindings_(web_contents, this) {}
...
void AppRuntimeWebViewHostImpl::DecidePolicyForResponse(
    bool is_main_frame,
    int32_t status_code,
    const std::string& url,
    const std::string& status_text,
    DecidePolicyForResponseCallback callback) {
  if (webview_delegate_)
    std::move(callback).Run(webview_delegate_->DecidePolicyForResponse(
        is_main_frame, status_code, url, status_text));
}
