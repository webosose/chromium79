...
#include "neva/pal_media_service/webos/umediaclient_impl.h"
...
#include "neva/pal_media_service/webos/media_util.h"
#include "neva/pal_media_service/webos/system_media_manager.h"
#include "neva/pal_media_service/webos/umedia_info_util_webos.h"
...
UMediaClientImpl::UMediaClientImpl(
    ...
    : duration_(0.0f),
      ...
      luna_service_client_(base::LunaServiceClient::PrivateBus),
...
void UMediaClientImpl::Load(bool video,
  ...
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DVLOG(1) << " url=" << url << " payload=" << payload;
...
void UMediaClientImpl::SetPlaybackVolume(double volume, bool forced) {
  ...
  int volume_level = static_cast<int>(volume * 100);
  int duration = 0;
...
void UMediaClientImpl::Suspend(mojom::SuspendReason reason) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  bool force_unload = false;
  DVLOG(1) << " - MediaId: " << MediaId();
...
void UMediaClientImpl::Resume() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DVLOG(1) << " - MediaId: " << MediaId() << " loading_state=" << loading_state_
...
void UMediaClientImpl::SetPreload(Preload preload) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DVLOG(1) << " app_id_=" << app_id_ << " preload=" << preload;
...
bool UMediaClientImpl::SetDisplayWindow(const gfx::Rect& outRect,
  ...
  if (!buffering_state_have_meta_data_) {
    DVLOG(1) << " - setDisplayWindow is aborted";
    return false;
  }
...
void UMediaClientImpl::DispatchPreloadCompleted() {
  ...
  if (IsNotSupportedSourceInfo()) {
    ...
    system_media_manager_->SourceInfoUpdated(has_video_, has_audio_);
    if (!buffering_state_cb_.is_null()) {
      DVLOG(2) << " buffering_state_have_meta_data_="
...
void UMediaClientImpl::DispatchError(int64_t errorCode,
                                     const std::string& errorText) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DVLOG(1) << " MediaId=" << MediaId() << " errorCode=" << errorCode
           << " msg=" << errorText;
...
void UMediaClientImpl::LoadInternal() {
  ...
  DVLOG(1) << " call uMediaServer::uMediaClient::loadAsync()";
...
bool UMediaClientImpl::LoadAsyncInternal(const std::string& uri,
  ...
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DVLOG(1) << " loading_state_ " << loading_state_ << " -> "
