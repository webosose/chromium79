...
class WebOSSystemInjection : public gin::Wrappable<WebOSSystemInjection>,
 ...
 public:
  ...
  WebOSSystemInjection(blink::WebLocalFrame* frame);
...
bool WebOSSystemInjection::DidRunOnCloseCallback() {
  return !WebOSServiceBridgeInjectionExtension::HasWaitingRequests();
}
...
void WebOSSystemInjection::SetAppInClosing() {
  ...
  WebOSServiceBridgeInjectionExtension::SetAppInClosing(true);
}
...
std::string WebOSSystemInjection::ServiceCall(
  ...
  if (!WebOSServiceBridgeInjectionExtension::IsClosing())
    return std::string("false");
