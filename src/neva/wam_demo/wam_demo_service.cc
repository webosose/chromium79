...
char kAllowLocalResourceLoad[] = "allowLocalResourceLoad";
...
char kDetachGroup[] = "detachGroup";
char kDisableBackButton[] = "disableBackButton";
char kDisableInspectablePage[] = "disableInspectablePage";
char kDisableWebSecurity[] = "disableWebSecurity";
char kDoNotTrack[] = "doNotTrack";
char kEnableInspectablePage[] = "enableInspectablePage";
char kFocusGroupLayer[] = "focusGroupLayer";
char kFocusGroupOwner[] = "focusGroupOwner";
char kForegroundApp[] = "foregroundApp";
char kGetDevToolsEndpoint[] = "getDevToolsEndpoint";
char kGetDocumentTitle[] = "getDocumentTitle";
char kGetDocumentUrl[] = "getDocumentUrl";
char kGetPid[] = "getPid";
...
char kIsProfileCreated[] = "isProfileCreated";
...
char kSetDisallowScrollbarsInMainFrame[] = "setDisallowScrollbarsInMainFrame";
char kSetExtraWebSocketHeader[] = "setExtraWebSocketHeader";
...
char kSimulateNetworkState[] = "simulateNetworkState";
...
char kEnableUnlimitedMediaPolicy[] = "enableUnlimitedMediaPolicy";
char kDisableUnlimitedMediaPolicy[] = "disableUnlimitedMediaPolicy";
...
char kDocumentTitle[] = "documentTitle";
char kDocumentUrl[] = "documentUrl";
char kCanGoBackAbility[] = "canGoBackAbility";
char kCursorUpdated[] = "cursorUpdated";
char kFirstContentfulPaintTime[] = "firstContentfulPaintTime";
char kFirstImagePaintTime[] = "firstImagePaintTime";
char kFirstMeaningfulPaintTime[] = "firstMeaningfulPaintTime";
char kFirstPaintTime[] = "firstPaintTime";
char kKeyboardVisibility[] = "keyboardVisibility";
char kLoadingEndTime[] = "loadingEndTime";
char kLoadFailed[] = "loadFailed";
char kLoadFinished[] = "loadFinished";
char kNonFirstMeaningfulPaintTime[] = "nonFirstMeaningfulPaintTime";
char kPidRequested[] = "pidRequested";
char kPidUpdated[] = "pidUpdated";
char kProcessGone[] = "processGone";
char kProfileCreated[] = "profileCreated";
...
char kCmd[] = "cmd";
char kConnected[] = "connected";
char kDisable[] = "disable";
char kDisallow[] = "disallow";
char kEnable[] = "enable";
char kFontFamily[] = "font_family";
char kFramelessWindow[] = "frameless_window";
char kFullScreen[] = "full_screen";
char kGreenColor[] = "green";
char kGroupName[] = "group_name";
char kHeaderName[] = "header_name";
char kHeaderValue[] = "header_value";
char kHeight[] = "height";
char kInputRegion[] = "input_region";
char kIsOwner[] = "is_owner";
char kJSCode[] = "javascript_code";
char kKeyMask[] = "key_mask";
char kKeySym[] = "key_sym";
char kMediaCodecCapability[] = "media_codec_capability";
char kName[] = "name";
char kNetworkQuietTimeout[] = "network_quiet_timeout";
char kOpacity[] = "opacity";
char kPosX[] = "pos_x";
char kPosY[] = "pos_y";
char kProfile[] = "profile";
char kProxyBypassList[] = "proxy_bypass_list";
char kProxyEnabled[] = "proxy_enabled";
char kProxyLogin[] = "proxy_login";
char kProxyPassword[] = "proxy_password";
char kProxyPort[] = "proxy_port";
char kProxyServer[] = "proxy_server";
char kRedColor[] = "red";
...
void WamDemoService::Launch(const std::string& appid, const std::string& appurl,
  ...
  BlinkView* webpage =
      new BlinkView(full_appid, params.width, params.height, this, profile);
  ...
  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kHidden))
    webapp->Show();
  appslist_.push_back(
      WamDemoApplication(appid, appurl, webapp, webpage, profile));
  ...
  if (app_it != appslist_.end()) {
    ...
    if (cmd == command::kChangeUrl) {
      ...
      const std::string zoom_str = base::NumberToString(
          (int)std::floor(app_it->page_->GetZoomFactor() * 100.f + 0.5));
      if (!zoom_str.empty()) {
      ...
      std::string command = response::kDocumentTitle;
      command += ':' + webpage->DocumentTitle();
      EmulatorSendData(command, app_it->appid_);
    } else if (cmd == command::kGetDocumentUrl) {
      std::string command = response::kDocumentUrl;
      command += ':' + webpage->GetUrl();
      EmulatorSendData(command, app_it->appid_);
      ...
      if (UnpackBool(value, argument::kDisable, disable))
        webpage->SetBackHistoryKeyDisabled(disable);
      else
      ...
      webpage->SetSSLCertErrorPolicy(neva_app_runtime::SSL_CERT_ERROR_POLICY_IGNORE);
      ...
      webpage->SetAllowLocalResourceLoad(true);
    } else if (cmd == command::kDisableWebSecurity) {
      webpage->SetWebSecurityEnabled(false);
      ...
      if (UnpackString(value, argument::kHeaderName, header) &&
        ...
        LOG(INFO) << __func__ << "(): Invalid params";
    } else if (cmd == command::kSetProfile) {
      ...
      std::string command = response::kProfileCreated;
      base::FilePath path =
          webpage->GetWebContents()->GetBrowserContext()->GetPath();
      command += ':' + base::MakeAbsoluteFilePath(path).value() + ':' +
                 std::string(base::PathExists(path) ? "yes" : "no");
      EmulatorSendData(command, appid);
      ...
      const std::string pid =
          base::NumberToString(app_it->page_->RenderProcessPid());
      if (!pid.empty()) {
      ...
      neva_app_runtime::WidgetState state = neva_app_runtime::WidgetState::UNINITIALIZED;
      ...
      if (!UnpackBool(value, argument::kDisallow, disallow))
        ...
        webpage->SetDisallowScrollbarsInMainFrame(disallow);
      ...
      const neva_app_runtime::WidgetState new_state = webapp->GetWindowHostState();
      ...
      const neva_app_runtime::WidgetState state = webapp->GetWindowHostStateAboutToChange();
      ...
      if (UnpackString(value, argument::kUserStyleSheet, user_stylesheet)) {
        ...
        webapp->SetCustomCursor(neva_app_runtime::CustomCursorType::kPath,
      ...
      if (UnpackString(value, argument::kVisibilityState, visibility_state)) {
        if (visibility_state.compare("Visible") == 0)
          webpage->SetVisibilityState(
              neva_app_runtime::WebPageVisibilityState::WebPageVisibilityStateVisible);
        else if (visibility_state.compare("Hidden") == 0)
          webpage->SetVisibilityState(
              neva_app_runtime::WebPageVisibilityState::WebPageVisibilityStateHidden);
        else if (visibility_state.compare("Launching") == 0)
          webpage->SetVisibilityState(
              neva_app_runtime::WebPageVisibilityState::WebPageVisibilityStateLaunching);
      }
      else
      ...
      if (UnpackUInt(value, argument::kKeyMask, key_mask))
        webapp->SetGroupKeyMask(static_cast<neva_app_runtime::KeyMask>(key_mask));
    } else if (cmd == command::kSetKeyMask) {
      unsigned key_mask = 0;
      bool set = false;
      if (UnpackUInt(value, argument::kKeyMask, key_mask) &&
          UnpackBool(value, argument::kSet, set))
        webapp->SetKeyMask(static_cast<neva_app_runtime::KeyMask>(key_mask), set);
    } else if (cmd == command::kEnableInspectablePage) {
      webpage->EnableInspectablePage();
    } else if (cmd == command::kDisableInspectablePage) {
      webpage->DisableInspectablePage();
      ...
      command += ',';
      command.append(base::NumberToString(webpage->DevToolsPort()));
...
void WamDemoService::OnWindowClosing(WebAppWindowImpl* window) {
  ...
  if (app_it != appslist_.end()) {
    ...
    delete app_it->page_;
    app_it->win_->DetachWebContents();
    delete app_it->win_;
    delete app_it->profile_;
    appslist_.erase(app_it);
  }
}

void WamDemoService::CursorVisibilityChanged(WebAppWindowImpl* window,
                                            bool visible) {
  auto app_it = std::find_if(appslist_.begin(), appslist_.end(),
      [window](WamDemoApplication& app) { return app.win_ == window; });

  if (app_it != appslist_.end()) {
    std::string command = response::kCursorUpdated;
    command += ":";
    command += (visible ? "visible" : "hidden");
    EmulatorSendData(command, app_it->appid_);
  }
}

void WamDemoService::OnPerformanceEvent(neva_app_runtime::WebViewBase* view,
                                        const std::string& type) {
  auto app_it = std::find_if(appslist_.begin(), appslist_.end(),
      [view](WamDemoApplication& app) { return app.page_ == view; });

  if (app_it != appslist_.end()) {
    base::TimeDelta delta = base::Time::Now() - app_it->creation_time_;
    LOG(INFO) << "Application " << app_it->url_ << " " << type << " is " <<
        delta.InMilliseconds() << " (ms)";
    std::string command = type;
    command += ":";
    command += base::NumberToString(delta.InMilliseconds());
    EmulatorSendData(command, app_it->appid_);
  }
}

void WamDemoService::OnDidLoadingEnd(neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kLoadingEndTime);
}

void WamDemoService::OnDidFirstPaint(neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstPaintTime);
}

void WamDemoService::OnDidFirstContentfulPaint(
    neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstContentfulPaintTime);
}

void WamDemoService::OnDidFirstImagePaint(neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstImagePaintTime);
}

void WamDemoService::OnDidFirstMeaningfulPaint(
    neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstMeaningfulPaintTime);
}

void WamDemoService::OnDidNonFirstMeaningfulPaint(
    neva_app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kNonFirstMeaningfulPaintTime);
}

void WamDemoService::OnTitleChanged(neva_app_runtime::WebViewBase* view,
...
void WamDemoService::OnRenderProcessGone(neva_app_runtime::WebViewBase* view) {
...
void WamDemoService::OnRenderProcessCreated(
    neva_app_runtime::WebViewBase* view) {
  SendCommandWithView(view, response::kAppStarted);
}

void WamDemoService::OnDocumentLoadFinished(
    neva_app_runtime::WebViewBase* view) {
  auto app_it = std::find_if(
      appslist_.begin(), appslist_.end(),
      [view](WamDemoApplication& app) { return app.page_ == view; });
...
void WamDemoService::OnLoadFailed(neva_app_runtime::WebViewBase* view) {
  SendCommandWithView(view, response::kLoadFailed);
}

bool WamDemoService::SendCommandWithView(neva_app_runtime::WebViewBase* view,
...
void WamDemoService::LaunchApp(const std::string& value,
  ...
  if (UnpackFloat(value, argument::kNetworkQuietTimeout,
                  network_quiet_timeout))
    webpage->SetNetworkQuietTimeout(network_quiet_timeout);
  ...
  if (UnpackString(value, argument::kGroupName, group_name)) {
    group_name = wam_demo_app_prefix_ + group_name;

    bool is_owner;
    if (UnpackBool(value, argument::kIsOwner, is_owner)) {
      if (is_owner) {
        LOG(INFO) << __func__ << "(): create group " << group_name;
        ...
        webapp->CreateGroup(config);
      } else {
        LOG(INFO) << __func__ << "(): attach to group " << group_name;

        webpage->SetTransparentBackground(true);
        webapp->AttachToGroup(group_name, kDefaultLayerName);
      }
    }
  } else
  ...
  appslist_.push_back(WamDemoApplication(appid, appurl, webapp, webpage, profile));
}
