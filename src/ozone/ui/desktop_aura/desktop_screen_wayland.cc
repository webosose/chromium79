...
display::Display DesktopScreenWayland::GetDisplayByDisplayId(int display_id) const {
...
display::Display DesktopScreenWayland::GetPrimaryDisplay() const {
  ...
  int display_id = std::stoi(getenv("DISPLAY_ID"));
  for (auto& display : displays_)
    if (display.id() == display_id)
      return display;
  return displays_.front();
...
void DesktopScreenWayland::OnScreenChanged(const std::string& display_id,
  ...
  int displayid = std::stoi(display_id);
