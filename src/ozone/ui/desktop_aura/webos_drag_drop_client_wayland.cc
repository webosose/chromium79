...
int WebosDragDropClientWayland::StartDragAndDrop(
  ...
  if (source == ui::DragDropTypes::DRAG_EVENT_SOURCE_TOUCH &&
      data->provider().GetDragImage().size().IsEmpty())
    return 0;
  ...
  drag_image_final_bounds_for_cancel_animation_ =
      gfx::Rect(start_location - drag_data_->provider().GetDragImageOffset(),
                drag_data_->provider().GetDragImage().size());
  drag_image_ =
      std::make_unique<DragImageView>(source_window->GetRootWindow(), source);
  drag_image_->SetImage(drag_data_->provider().GetDragImage());
  drag_image_offset_ = drag_data_->provider().GetDragImageOffset();
...
void WebosDragDropClientWayland::OnTouchEvent(ui::TouchEvent* event) {
  if (!IsDragDropInProgress())
    return;
  ...
  if (pending_long_tap_) {
    ...
    if (!should_block_during_drag_drop_) {
      ...
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE,
          base::Bind(&WebosDragDropClientWayland::ForwardPendingLongTap,
...
void WebosDragDropClientWayland::ForwardPendingLongTap() {
