...
class MEDIA_EXPORT ExternalRenderer : public Renderer,
 ...
 private:
  ...
  void AudioBufferReady(AudioDecoderStream::Status status,
                        const scoped_refptr<AudioBuffer> frame);
  void VideoFrameReady(VideoDecoderStream::Status status,
  ...
  void OnPlayerEvent(MediaPlatformAPI::PlayerEvent e);
