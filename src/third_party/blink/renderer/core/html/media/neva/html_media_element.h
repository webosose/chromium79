...
const String HTMLMediaElement<original_t>::cameraId() const {
  const original_t* self(static_cast<const original_t*>(this));
  DCHECK(RuntimeEnabledFeatures::CustomEventExtensionEnabled());
...
WebString HTMLMediaElementExtendingWebMediaPlayerClient<original_t>::Cookies()
  ...
  // Request restricted cookie manager
  if (!backend.is_bound() || backend.encountered_error()) {
    doc->GetInterfaceProvider()->GetInterface(mojo::MakeRequest(&backend));
  }

  String value;
  backend->GetCookiesString(self->currentSrc(), doc->SiteForCookies(),
                            doc->GetSecurityOrigin(), &value);
