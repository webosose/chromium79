...
void RenderWidget::OnActivateCompositor() {
  if (!compositor_never_visible_)
    layer_tree_view_->SetVisible(!is_hidden());
}
...
void RenderWidget::NotifyVizFMPSwap(bool is_first_contentful_paint,
                                    bool did_reset_container_state) {
  layer_tree_host_->QueueSwapPromise(std::make_unique<VizFMPPromise>(
      is_first_contentful_paint, did_reset_container_state));
}
#endif
